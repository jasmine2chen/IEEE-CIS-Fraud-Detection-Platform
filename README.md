...
| `make train [MODEL=...]` | Train one model (default: xgboost) |
| `make tune [MODEL=...] [TRIALS=...]` | Optuna HPO (default: 50 trials) |
| `make tune-then-train [MODEL=...]` | Sequential tune → write config → retrain |
| `make register` | Registration is automatic post-training |
| `make promote [MODEL=...]` | Promote latest version to @champion |
| `make batch-score [MODEL=...] [OUTPUT=...]` | Offline batch scoring from @champion |
| `make monitor [MODEL=...] [OUTPUT=...]` | Run drift detection against current batch |
| `make pipeline-run [MODEL=...] [TRIALS=...]` | Full Prefect orchestration flow |
| `make stack-up` | Start full stack: API + MLflow + Grafana + PostgreSQL |
...
| `make docker-run` | Run containerised API + MLflow |
...
fraud_detection/
├── .github/workflows/
│   ├── ci.yml                      GitHub Actions — pytest matrix (3.9, 3.11) + mypy
│   └── cd.yml                      CD — integration tests → Docker build → deploy notify
├── .pre-commit-config.yaml         black, ruff, isort hooks
├── api/
│   ├── main.py                     FastAPI endpoints + lifespan model loading (registry-first)
│   └── schemas.py                  Pydantic v2 request/response models
├── configs/model_config.yaml       Single source of truth — hyperparams, RFE indices,
│                                   serving threshold
├── docs/
│   ├── architecture.md             Mermaid system diagrams
│   └── production.md               Monitoring, retraining, cost analysis, failure modes
├── monitoring/
│   ├── init_db.sql                 PostgreSQL schema — drift_metrics, prediction_logs
│   └── grafana/
│       ├── provisioning/           Auto-configured datasources + dashboard providers
│       └── dashboards/             Pre-built fraud monitoring dashboard
├── notebooks/
│   └── 01_eda_and_baselines.ipynb  EDA + all 4 models side-by-side + SHAP analysis
├── pipelines/
│   └── training_pipeline.py        Prefect flow — tune → train → register → promote
├── src/
│   ├── config.py                   YAML config loader
│   ├── registry.py                 MLflow Model Registry — register, @champion alias, load
│   ├── batch_score.py              Offline batch scoring pipeline
│   ├── train.py                    Training orchestrator — 4 model types, tuned/untuned paths
│   ├── tune.py                     Optuna HPO + RFE — XGBoost 4-step, neural 2-phase hybrid
│   ├── data_prep/data_loader.py    Load, merge, dtype-downcast, memory-optimise
│   ├── features/build_features.py  Magic UID, D-norm, velocity aggregations, sklearn Pipeline
│   ├── models/
│   │   ├── tree_models.py          XGBoost + custom FPR eval metric
│   │   ├── mlp_tree.py             MLP encoder (FocalLoss + FPR early stop) → XGBoost
│   │   ├── gnn_tree.py             GraphSAGE (7-day temporal graph) → XGBoost
│   │   └── transformer_tree.py     TabTransformer (Pre-LN, CLS token) → XGBoost
│   ├── monitoring/
│   │   └── drift.py                Evidently drift detection + PostgreSQL persistence
│   └── evaluation/
│       ├── metrics.py              FPR sweep, partial AUC@5%, dollar recall
│       └── plots.py                Calibration, SHAP, score distribution visualisations
├── tests/                          pytest suite — mocked I/O, no real data in CI
├── Dockerfile                      Multi-stage production image (~400MB runtime)
├── docker-compose.yml              Full stack: API + MLflow + PostgreSQL + Grafana
...
| Layer | Technology |
|---|---|
| Models | XGBoost, PyTorch, PyTorch Geometric |
...
| Orchestration | Prefect |
| Batch scoring | pandas, Parquet (pyarrow) |
| Drift monitoring | Evidently, PostgreSQL, Grafana |
